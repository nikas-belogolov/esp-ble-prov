...
import { ProvisionerError, ESPProvisionerOptions } from './types';
...
export class ESPProvisioner {
  ...
  getCharacteristic(ep_name: string): BluetoothRemoteGATTCharacteristic {
    ...
    if (!char)
      throw new ProvisionerError(`Characteristic "${ep_name}" not found.`);
    return char;
  }
  ...
  async writeValue(ep_name: string, data: Uint8Array) {
    return this.getCharacteristic(ep_name).writeValue(data as BufferSource);
  }
  ...
  private async discoverCharacteristics() {
    ...
    if (!characteristics)
      throw new ProvisionerError("Couldn't get device characteristics.");
    ...
    for (const char of characteristics) {
      try {
        ...
        const name = await descriptor
          .readValue()
          .then((v) => textDecoder.decode(v));
        this.endpoints.set(name, char);
      } catch (error) {
        console.warn(
          `Failed to get descriptor for characteristic ${char.uuid}`,
          error
        );
      }
    }
   ...
   */
  async connect(options?: { filters: BluetoothLEScanFilter[] }) {
    if (this.isConnected) {
    ...
    try {
      ...
      this.service =
        (await this.server?.getPrimaryService(this.serviceUUID)) || null;
  ...
  async establishSession() {
    ...
    try {
      ...
      if (
        this.security instanceof Security1 ||
        this.security instanceof Security2
      ) {
      ...
      throw error instanceof Error
        ...
        : new ProvisionerError(
            'Failed to establish session with unknown error'
          );
    }
  }
  ...
  async ctrlReset() {
    ...
    try {
      ...
      throw error instanceof Error
        ? new ProvisionerError(
            `Failed to reset provisioning state: ${error.message}`
          )
        : new ProvisionerError(
            'Failed to reset provisioning state with unknown error'
          );
    }
  }
  ...
  async scan(
    ...
    try {
      ...
      throw error instanceof Error
        ? new ProvisionerError(
            `Failed to scan for WiFi networks: ${error.message}`
          )
        : new ProvisionerError(
            'Failed to scan for WiFi networks with unknown error'
          );
    }
  }
  ...
  async getWiFiStatus() {
    ...
    try {
      ...
      throw error instanceof Error
        ? new ProvisionerError(
            `Failed to get WiFi STA status: ${error.message}`
          )
        : new ProvisionerError(
            'Failed to get WiFi STA status with unknown error'
          );
    }
  }
   ...
   */
  async setWiFiConfig(options: ICmdSetConfig) {
    this.ensureConnected();
    ...
    try {
      ...
      throw error instanceof Error
        ? new ProvisionerError(
            `Failed to set WiFi credentials: ${error.message}`
          )
        : new ProvisionerError(
            'Failed to set WiFi credentials with unknown error'
          );
    }
  }
  ...
  async applyWiFiConfig() {
    ...
    try {
      ...
      throw error instanceof Error
        ? new ProvisionerError(
            `Failed to apply WiFi credentials: ${error.message}`
          )
        : new ProvisionerError(
            'Failed to apply WiFi credentials with unknown error'
          );
    }
  }
  ...
  async waitForWiFiStatus(timeout: number = 60000) {
    ...
    try {
      do {
        ...
        if (
          status.staState === WifiStationState.ConnectionFailed ||
          (status.attemptFailed && status.attemptFailed.attemptsRemaining === 0)
        ) {
          throw new ProvisionerError(
            'WiFi connection failed: No attempts remaining'
          );
        }

        if (Date.now() - startTime > timeout) {
          throw new ProvisionerError(
            `WiFi connection timed out after ${timeout}ms`
          );
        }
      ...
      if (!this.isConnected) {
        throw new ProvisionerError(
          'Device disconnected while waiting for WiFi status'
        );
      }
    } catch (error) {
      ...
      throw error instanceof Error
        ? new ProvisionerError(
            `Error while waiting for WiFi status: ${error.message}`
          )
  ...
  async writeValueToEndpoint(ep_name: string, value: Uint8Array) {
    ...
    try {
      ...
      throw error instanceof Error
        ? new ProvisionerError(
            `Failed to write to endpoint ${ep_name}: ${error.message}`
          )
  ...
  async readValueFromEndpoint(ep_name: string) {
    ...
    try {
      const value = await this.readValue(ep_name);
