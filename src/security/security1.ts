...
export default class Security1 extends Security {
  ...
  private pop: Uint8Array<ArrayBuffer> | undefined;
  ...
  private async generateX25519Key() {
    ...
    this.publicKeyBuffer = new Uint8Array(
  ...
  private async deriveSharedKey(publicKey: CryptoKey) {
    if (!this.privateKey)
      throw new ProvisionerError(
        'Generate X25519 key pair before deriving shared key.'
      );
    ...
    sharedKey.set(
      await crypto.subtle
        .deriveBits({ name: 'X25519', public: publicKey }, this.privateKey, 256)
        .then((v) => new Uint8Array(v))
    );

    if (this.pop) {
      const popHashBuffer = await crypto.subtle
        .digest('SHA-256', this.pop)
  ...
  async setup0Response(response: Uint8Array) {
    ...
    const { devicePubkey, deviceRandom } = sessionData.sec1?.sr0!;

    const devicePublicKey = await crypto.subtle.importKey(
      'raw',
      devicePubkey as BufferSource,
      { name: 'X25519' },
      true,
      []
    );

    const sharedSecret = await this.deriveSharedKey(devicePublicKey);
  ...
  setup1Request(): Uint8Array {
    ...
    const encryptedDevicePublicKey = this.cipher?.encrypt(
      this.devicePublicKeyBuffer
    );

    const sessionData = SessionData.create({
      secVer: SecSchemeVersion.SecScheme1,
      sec1: Sec1Payload.create({
  ...
  setup1Response(response: Uint8Array): void {
    ...
    const sessionData = SessionData.decode(response);
    const deviceVerifyDataDecrypted = this.cipher?.decrypt(
      sessionData.sec1?.sr1?.deviceVerifyData!
    );
